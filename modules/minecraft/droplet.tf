resource "digitalocean_droplet" "minecraft_test_server" {
  ...
  backups            = false
  ipv6               = true
  name               = "minecraft-test-server-01"

  count = var.servers_count

  provisioner "local-exec" {
    command = "sleep 30 && cd ../../ansible && ansible-galaxy install -r roles/requirements.yml --roles-path=roles/ -f && ansible-playbook -u root -i '${self.ipv4_address},' --private-key ~/.ssh/id_rsa minecraft-java.yml"
  }
